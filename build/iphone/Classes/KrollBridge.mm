...
@implementation TitaniumIntentTestObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_titaniumintenttest);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any TitaniumIntentTest patches in JS before a context is loaded... nice for 
...
{
	// create TitaniumIntentTest global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_titaniumintenttest = [[TitaniumIntentTestObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_titaniumintenttest];
	
	NSString *_titaniumintenttestNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _titaniumintenttestNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_titaniumintenttest valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.titaniumintenttest.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
